...
顺带修掉一处 enforce 相关隐患：原先本地只要有 `storeProductId` 就直接下单，而 enforce 之前上架的商品**可能从未登记**——那样订单会在你们侧被挡，我们这边看起来却一切正常。现在每次下单前都会过一遍「登记 → 上架」同步，登记被拒就回 `402 NOT_LISTED`。

### 12.7 `GET /api/assets/registry/{ref}` 仍未接
