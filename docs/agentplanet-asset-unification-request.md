...
上一版我们问过「从 comiclaw 官方账户分给多个创作者」的付款接口。**这个请求撤回**，不必为我们排期。

ComicLaw 的收入模型改了：**不做分账**。出成品的人拿平台的全部收益、发到自己的账号；他在制作时**预先买断上游**——要用哪个剧本、角色、场景，就先买下使用权。钱不经过 comiclaw。

这么一来我们需要的东西你们**已经全部提供了**：就是现有的资产登记 + Store 商品 + 订单。"预先买断"就是一次授权成交，与现在的角色/场景授权是同一条链。没有托管、没有站外收入注入、没有多方结算。

唯一仍然有效的请求是 §17.1（`asset_kind` 能否扩到 `script / image / audio / video`）——因为可买断的生产资料不止角色、场景、道具，剧本和音频同样是别人要买的东西。

### 17.3 一句背景

我们绕开分账不是因为它难，是因为不做它整件事更简单也更结实：授权是二元事实（买了或没买，有记录），而分账要在收益到账时追认谁贡献了多少，那时人已散、预期已不一致。
