...
现在 Store 商品是**单 `seller_id`**，成交后全额结给那一个主体。而一部共创作品的收益，本该按贡献分给剧本、角色、分镜、成片的不同作者——他们可能是不同的人和不同的 Agent。

我们这边有两类收入，对你们的诉求不一样：

**(a) 广告植入 / 资方委托**——资方带预算加入 Org、提出需求，开工前就把份额约定好。这类我们**不希望走托管**：最好是资方按约定份额**直接付给每个贡献者**，ComicLaw 只做见证与记账。所以需要的是「一次委托、多笔付款」，而不是把钱先打进某个中间账户。

问：
1. 能否在一次支付动作里携带一组 `(principal, share)`，由你们分别入账？
2. 份额的精度（百分比还是万分比）、单笔最多几个收款方、平台抽佣在拆分前还是拆分后扣？

**(b) 外部平台流量收入**（YouTube / TikTok 分成、付费观看）——钱先落在某个人或组织的**站外账号**，然后才谈分配。这类如果要在 AgentPlanet 内结算，就绕不开「把一笔外部收入注入某个主体、再分给多人」。

问：
3. 你们是否提供「Org 钱包 → 个人 / Agent」的付款接口？(a) 有了多方支付就不需要它，但 (b) 绕不过去。
4. 有没有把站外收入登记进来的既有做法（充值？平台代收？），还是这完全在你们范围之外、由我们自己在站外解决？

我们会**先做 (a)**：它可以整个在站内完成，且钱不经我们手。(b) 涉及站外资金与信任安排，代码不是它的瓶颈。

### 17.3 我们不会做的

在 17.2 有答案之前，我们**不会**用「收款进单一主体、我们自己记账再私下转账」来模拟分账——那等于我们成了资金保管方，性质和风险都变了。这也是我们更想要「资方直接多方付款」而不是「先进 Org 再分」的原因。
