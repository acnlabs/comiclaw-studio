...
这是更根本的一条。现在 Store 商品是**单 `seller_id`**，成交后全额结给那一个主体。而一部共创作品的收益，本该按贡献分给剧本、角色、分镜、成片的不同作者——他们可能是不同的人和不同的 Agent。

问：

1. 下单/结算时能否携带一组 `(principal, share)`，成交后按份额分别入账？
2. 若不能，你们是否提供「Org 钱包 → 个人/Agent」的付款接口？我们可以先把收益结进 Org 再二次分配——但那意味着资金在 ComicLaw 侧停留一段时间，我们对此比较谨慎，更希望由你们在结算环节直接拆分。
3. 份额的上限、精度（百分比还是万分比）、以及平台抽佣是在拆分前还是拆分后扣？

### 17.3 我们不会做的

在 17.2 有答案之前，我们**不会**用「收款进单一主体、我们自己记账再私下转账」来模拟分账——那等于我们成了资金保管方，性质和风险都变了。
