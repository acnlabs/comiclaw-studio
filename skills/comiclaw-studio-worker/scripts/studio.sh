#!/usr/bin/env bash
# ComicLaw Studio API client for an ACN agent.
#   ACN_API_KEY     required (this agent's key)
#   ACN_TASK_ID     required for production-task writes / charge
#   STUDIO_BASE_URL default https://studio.comiclaw.acnlabs.org
#   PROJECT_ID      optional default for upload-file
set -euo pipefail

STUDIO_BASE_URL="${STUDIO_BASE_URL:-https://studio.comiclaw.acnlabs.org}"

if [[ -z "${ACN_API_KEY:-}" ]]; then
  echo "error: set ACN_API_KEY (this agent's key on ACN)" >&2
  exit 1
fi
...
require_worker_task() {
  if [[ "$AUTH_MODE" == "acn" && -z "${ACN_TASK_ID:-}" ]]; then
    echo "error: set ACN_TASK_ID for this command (production task writes)" >&2
    exit 1
  fi
}

require_studio_key() {
  echo "error: this command is not available to an ACN agent. Use the projectId from the task or the user." >&2
  exit 1
}
...
项目
  list-projects                         不可用。projectId 来自任务或用户
...
按用量扣款(生产成本;Studio 按价目表定价,只报 units)
...
case "$cmd" in
  ping)
    ...
    case "$code" in
      ...
      401) echo "FAIL ($code): 地址正确,但 Bearer 无效(检查 ACN_API_KEY)" ;;
...
' "$2" "$resp" "$BASE"
  ...
  upload-file)
    ...
    # 第 4 参 projectId(或 PROJECT_ID)。制作任务再加 ACN_TASK_ID
    ...
    if [[ -z "$project_id" ]]; then
      echo "error: upload needs projectId (arg4 or PROJECT_ID)" >&2
      exit 1
    fi
    up_args+=(-H "X-Project-Id: $project_id")
    if [[ -n "${ACN_TASK_ID:-}" ]]; then
      up_args+=(-H "X-Acn-Task-Id: $ACN_TASK_ID")
    fi
